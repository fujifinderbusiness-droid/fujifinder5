...
export const AdminLogin: React.FC = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (!cleanEmail) {
      setErrorMessage('Masukkan email administrator.');
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (msg.includes('rate limit') || (error as any).status === 429) {
          msg = 'Terlalu banyak permintaan reset kata sandi. Mohon tunggu beberapa saat sebelum mencoba lagi.';
        }
        setErrorMessage(msg);
        return;
      }
      ...
      setForgotSuccessMessage(
        `Tautan pemulihan kata sandi telah dikirim ke ${cleanEmail}. Silakan periksa inbox atau spam email Anda dan klik tautan untuk membuat kata sandi baru.`
      );
    } catch (err: any) {
      setIsLoading(false);
      setErrorMessage(err.message || 'Gagal mengirim email pemulihan.');
    }
  };
    ...
    <div className="min-h-screen bg-[#0E0E0E] text-[#FAF9F6] flex flex-col justify-between p-4 sm:p-6 lg:p-8 selection:bg-white selection:text-black">
      ...
      <div className="w-full max-w-md mx-auto flex items-center justify-between pt-2 sm:pt-4">
        ...
        >
          ...
          <span>{isForgotPassword ? 'Kembali ke Login' : 'Kembali ke Website'}</span>
        </button>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[#1A1A1A] border border-[#2A2A2A] rounded-full text-[10px] text-[#AAA] tracking-wider uppercase">
          ...
          <span>{isForgotPassword ? 'Pemulihan Akun' : 'Area Terbatas'}</span>
        </div>
      </div>

      {/* Main Login / Forgot Password Card */}
        ...
        <div className="bg-[#141414] border border-[#262626] rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          ...
          <div className="text-center space-y-2">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white text-[#0E0E0E] mb-2 shadow-sm">
              {isForgotPassword ? <KeyRound className="w-6 h-6" /> : <Camera className="w-6 h-6" />}
            </div>

            <h1 className="font-serif text-2xl sm:text-3xl font-normal tracking-tight text-white">
              {isForgotPassword ? 'Lupa Kata Sandi' : 'FujiFinder CMS'}
            </h1>
            <p className="text-xs text-[#888] max-w-xs mx-auto leading-relaxed">
              {isForgotPassword 
                ? 'Masukkan email admin Anda untuk menerima tautan pemulihan kata sandi melalui Supabase Authentication.'
          ...
          {forgotSuccessMessage && (
            <div className="p-4 bg-emerald-950/70 border border-emerald-800/80 rounded-xl text-xs text-emerald-300 flex items-start gap-2.5 text-left animate-in fade-in">
              ...
              <div className="space-y-1">
                <p className="font-medium text-emerald-200">Email Pemulihan Terkirim</p>
                <p className="text-emerald-300/90 leading-relaxed">{forgotSuccessMessage}</p>
              </div>
            </div>
          )}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[11px] text-[#888] hover:text-white transition-colors cursor-pointer"
                  >
                    Lupa kata sandi?
                  </button>
                </div>
